...
#include <ctype.h>
#define st_size 20
void verb_checker(char [st_size],char [st_size],char [st_size]);
int main() {
    char st1[st_size], st2[st_size], st3[st_size];
    ...
    while (1) {
        printf("verb1:");
        gets(st1);
        if (strpbrk(st1, "0123456789") == NULL) {
            break;
        } else {
    ...
    while (1) {
            printf("verb2:");
            gets(st2);
            if (strpbrk(st2, "0123456789") == NULL) {
                break;
            } else {
                printf("\a\nInvalid input detected !!!\n");
                printf("Hint: only alphabet char is valid a-z \n");
    ...
    while (1) {
            printf("verb3:");
            gets(st3);
            if (strpbrk(st3, "0123456789") == NULL) {
                break;
            } else {
                printf("\a\nInvalid input detected !!!\n");
                printf("Hint: only alphabet char is valid a-z \n");
    ...
    void verb_checker(char string[st_size], char string2[st_size], char string3[st_size]) {
